...
>
> 2026-09-08 P3 实施更新：P3-01=`1db8435`；P3-02A=`a438861`、P3-02B=`5164323`、P3-02C=`d7cc2c2`。全量测试发现情绪门控测试存在固定历史时间与墙钟混用，已由 `5df95bf` 消除漂移。P3 完成时 bot.db schema 为 v22；P3-01/P3-02 不再是待实现工作。
>
...
| bot.db schema | `backend/core/userdb.py` 当前版本 41；其他数据库有各自版本 |
...
| P0-04 | 槽位路由 + 模型/搜索实测 | A/B 已完成：`9720fca`/`0f2b64b`；C2 已实测（2026-09-12，各 10 条）：博查 10/10、平均 0.27s、51 域名；Tavily 10/10、平均 2.39s、55 域名；域名重合度约 0.02。结论：双轨分工（中文查询博查优先、非中文 Tavily 优先、互为回退），已接线并加回归 `tests/test_search_routing.py`；C1 模型实测已完成 |
...
- B：显式晚安先遵守用户结束意图，只给一段简短回应，不趁机追问。行程“去忙”是角色表达，用户需要帮助时基本可及。2026-09-11 用户更正：休息/睡眠状态不锁输入框，但前两条消息保持沉默；10 分钟内连续第 3 条消息将她吵醒并回复，唤醒后维持 30 分钟可聊窗口，窗口内新消息续期；沉默轮只存用户消息，不调用 LLM、不写空助手消息。
...
## 24. 2026-09-09 能力评估（执行：ZCode；审查：Codex）
...
| **Agent 执行** | **7.5** | 工具循环（熔断/预算/结构化错误）+ 技能通道 + 子代理并行 + 任务面板 + **定时/重试/产物落盘/聊天派活**（见 24.1） | 缺全局并发与 token 上限；执行轨迹只有粗日志，失败复盘弱 |
...
- **监控能力从 0 到有**：`watches` 表（schema v40 引入、v41 增加持久投递确认）+ `core/watchers.py` + 四个工具，只存 URL 与内容哈希、不存正文；变化经主动链通知，消耗共享额度，真正投递后才确认该次变化。
