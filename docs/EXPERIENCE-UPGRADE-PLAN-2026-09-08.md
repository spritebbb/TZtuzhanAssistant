...
>
> **与母文档的关系（2026-09-08 整合）**：本文档已并入《Zcode技术指导》体系——技术指导是唯一权威设计源（切片定义/契约/参数以它为准），本文档只承担**实施调度**职责：批次划分、执行口径、状态回写。技术指导各节的「方案完成/待实现」状态在批次落地时由本文档状态表统一回写提交号，技术指导正文不再逐节改状态，避免双文档漂移。冲突裁决：设计内容以技术指导为准，调度与执行口径以本文档为准。技术指导文件头已加本说明的指回链接。

## 0.1 执行口径（2026-09-08 用户逐项拍板）

| # | 问题 | 拍板 |
|---|---|---|
| 1 | 执行节奏 | **连续推进**：按批次序连做，批次 3/7/12/终局四个节点跑全量+信箱汇报；设计文档能自答的不打断用户 |
| 2 | 批次 13 阈值数值 | **按提议**：新用户前 3 天主动 `_IDLE_HOURS`=2h；惊喜 7 天/50%/120 分钟；快照首月 30 天即达；用户可在设置页改回 |
| 3 | 新大功能开关 | **默认开+可关**：每个大功能挂 `features.py` FLAG_DEFAULTS（设置页面板已有）；同时提供 `FEATURE_<任务语义>_ENABLED` env 作为部署级初值（13.6 双层口径：动态层覆盖 env 层）。注：本项拍板覆盖 §19 决策 8 的「默认 False」——那是体验收口前的旧口径，已在体验收口四片（028034d）对两个稳定开关翻为默认 True 并经用户确认 |
| 4 | 素材/台词内容 | **我写我审**：批次内零散文学素材（日程素材量级）ZCode 直接写直接用，Codex 总审查把关；人格卡级大改仍走 P1-01 三段流程 |
| 5 | 全量聚合频率 | **节点聚合**：针对性测试每批跑，8 分钟全量只在节点跑 |
| 6 | 前端范围 | **全部做完整**：涉及前端的功能（L03/F06/F07/L08B/L01 等）含组件测试一并交付 |
| 7 | LLM 预算 | **被动触发**：新增 LLM 调用全部复用既有任务槽位路由（extract/judge/chat），事件驱动，不新增每晚定时批量任务 |
| 8 | 完成定义 | **落库+全量绿即本轮完成**：13 批次全部落库+终局全量绿+状态表回写+信箱总汇报后由用户交 Codex 审查 |
| 9 | L06 外出与主动消息 | **接主动候选**：外出归来作为主动候选源接入 initiative（消耗共享额度）；设计文档只要求状态行，此项为用户追加拍板 |
| 10 | 外出频率与沉默权 | 概率 **0.25 按设计原文**；外出意图可以听用户意见调整（用户说「别出门」她可以留下）；**出门后可对用户保持沉默**，但**必须留下外出的提醒**（状态行/生活流可见，不失踪） |
...
批次16 流式输出改造（2026-09-09 立项）：16A 客户端缓存修复 + 16B 增量卫生流式 + 16C 工具轮正文流式
```
...
- **用户意见与沉默权（拍板 #10）**：外出决定可在事件产生前听用户当次意见（用户明说「别出门/今天别去」→ 当日候选作废，记 cooldown 不再抽）；出门后她可以不主动汇报（沉默权），但外出状态必须对用户可见——状态行 + 生活流 + 事件表三处落地，不失踪。
- **主动候选（拍板 #9）**：外出归来事件作为主动候选源接入 initiative（`_arbited_proactive` proposer 链），消耗共享每日额度；候选文案取真实事件 payload，失败静默。
...
| 批次 | 状态 | 提交号 | 日期 |
|---|---|---|---|
| 1 L06 | ZCode 已实现；Codex 针对性复验通过 | c4856eb | 2026-09-08 |
| 2 L03 | ZCode 已实现；Codex 修复真实事件失效入口的证据清理并复验 | 3025c49、0bec7c5 | 2026-09-08 |
| 3 G01 | 完成：底座、生产评分、期限消费、锚点/初历生产来源、解释层露出与可见遗忘（F07 完整气泡交互留批次 8） | e807658、c57a99d、475f71d、0911585、371c98d | 2026-09-08 |
| 4 F01 | 完成：素材授权来源 + 四类变体池 + 阶段门控/7 天冷却 + greeting/initiative 双接入（schema v26） | 18b7643 | 2026-09-08 |
| 5 G03 | 完成：开放问题全状态机 + JOB-1 复查 + 证据哈希 + 主动候选（schema v27） | 9301dd8 | 2026-09-08 |
| 6 G04 | 完成：双门槛求助 + 24h 过期 + 虚构产物 + API/聊天意图同函数（schema v28） | 9b35224 | 2026-09-08 |
| 7 F03+L04+F02 | 完成：心事门控 / 幽默记忆 / 素材互通（schema v29–v31）。节点一全量：后端 124 passed（8m30s）、前端 vue-tsc+74/74 | d9d169c、fcaa619、07500bc | 2026-09-08 |
| 8 F04+F05+F06+F07 | 完成：专注收尾投递箱 / 阅读地图 / 意图草稿 / 记忆生命周期交互（schema v32–v34） | 8f5023f、dffb205、6a017b8、1a181bb | 2026-09-08 |
| 9 L01 | 完成：URL 边界/EPUB 防炸/分段/去重/任务取消（schema v35）+ 前端网页导入入口 | 0eb4cad、361635d | 2026-09-08 |
| 10 L02 | 完成：subtype/大纲/观察日志（schema v36）+ 前端 subtype 选择、大纲编辑与观察界面 | d3c39f0、6116967 | 2026-09-08 |
| 11 L05+L07+L08 | 完成。执行：Codex。L05 领域信任（schema v37）；L07 审美/房间（schema v38）；L08 统一 VisualState、异常状态灯与工具收纳 | 635ff5e、b7fec73、c0a58f9 | 2026-09-09 |
| 12 L16+P3-05+§17 | 完成。执行：ZCode。L16 共享授权（schema v39）；P3-05 演化日志+本地统计（ADR）；§17.1 必要性门+注意力；§17.2 学习管线；§17.3 工具加固+LRU 缓存 | 32f646f、2f47f9e、e405e69、326db21、a9d82d9 | 2026-09-09 |
...
| 15 D 类离线件（P3-03/04、L09/L10/L12/L13/L14/L15） | **部分完成**（2026-09-21 更新）。P3-04 A–E 已落地且真实 `data/` 已迁移加密（明文备份 09-20 演练后删除）；**F 片加密备份已落库**（c6f256b）且真实备份/恢复演练 09-20 全绿。**L09 STT worker 骨架已落地**（帧协议/限额/取消/无模型 8 组回归 + 前端接线 + PWA 禁用态；模型下载 UI 未做，不装模型）。**L10 桌宠骨架已落地**（petWindow/displayAvoidance/petWindowState/pet.html + 设置开关 + Vitest；前台全屏 helper 未实装=避让 mock 模式，多屏/8 小时真机验收留空位）。P3-03、L12、L13、L14、L15 未开工 | A/B/C：94f0593、f43a1d2；D：9d90af0；E：e34bf56、633289b、2b83d83、46795b4、8954b5d、8c9bb62；F：c6f256b（09-12～18）；L09/L10：2026-09-21 落库 | 2026-09-10 ～ 2026-09-21 |
...
- **Q3 可观测性**：已完成。`core/telemetry.py` 使用安装级盐派生用户作用域哈希；chat→tool→OUT-1→session 以服务端 request_id 和前端 logical_message_id 串联；事件链保留 7 天、低基数日聚合保留 30 天且单人格每日事件有上限；设置页可立即关闭并清理，临时轮和 mock 不落盘；解释接口只返回高层步骤、真实来源类别和结果。
- **Q4 性能/迁移/故障**：已完成。`test_latency_budgets` 建立 p95≤100ms/80ms 本地预算并检查 2 万行 SQLite 查询计划；`test_old_database_upgrade` 保留 v40 去敏 fixture，覆盖 v41 升级、幂等、升级前快照、中断目标恢复与 reset；`test_job_recovery` 覆盖 scope 隔离、双 worker 认领、租约过期、时钟回拨、迟到完成、reset 删除及 provider/流式故障边界。
- **Q5 无障碍/e2e**：已完成。统一普通/紧急对话框的初始焦点、Tab 闭环、Escape 关闭与焦点返回；为设置、任务、聊天、记忆恢复等当前入口补齐语义名称和错误播报；关系与原因默认用一行短说明，展开后仍不显示等级、数字或进度条；自动检查深浅主题正文对比度，并用 Playwright 验证聊天、设置、200% 缩放、360px 窄屏和 reduced-motion。设备撤销与多人格房间选择属于批次15尚未实现的 D 类入口，本批次不虚报覆盖。
...
### 批次16：流式输出改造（2026-09-09 用户立项「先纳入计划」；三片独立提交）

完整契约见 [Zcode技术指导.md](Zcode技术指导.md) §25，本节只登记切片划分与依赖。

- **16A 客户端缓存修复（已实现并提交 c0c783d）**：`backend/api/config_api.py` 热重载先清 `llm._client_cache`、再把缓存内全部实例纳入统一关闭；回归 `tests/test_llm_client_cache.py`。修复前症状：设置页保存后每次 LLM 调用抛「Cannot send a request, as the client has been closed」，只能重启后端恢复。
- **16B 增量卫生流式**：新增 `backend/core/stream_hygiene.py`（分句 + 增量规则扫描，`inspect_reply` 的规则部分抽为 `scan_rules`）；改 `pipeline.py` 普通流式推流点为「累积 + 逐安全单元推送」，在保留卫生检查的前提下恢复打字机；`ChatView.vue` 识图回调补 `onReset`。
- **16C 工具轮正文流式**：`backend/tools/tool_loop.py` 最终答复轮从 `chat` 改 `chat_stream`（工具调用轮保持非流式），熔断/预算/结构化错误契约不变。

依赖与顺序：16A 独立，可先提交；16B 是主体，独立于 16C 验收；16C 依赖 16B 的增量出口。三片均需带 VERIFY，并按纪律单主题提交。
