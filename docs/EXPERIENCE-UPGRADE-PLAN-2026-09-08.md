...
| 15 D 类离线件（P3-03/04、L09/L10/L12/L13/L14/L15） | 进行中。P3-04 A/B 离线件完成：明文面 ADR、全后端 SQLite 统一连接入口、SQLCipher 4.12 临时目录导出与完整性/等价性 PoC；未迁移真实数据。其余训练/真机/账号环节按计划留运行记录空位 | 94f0593 | 2026-09-10 |
...
- **P3-04 A/B 片**：已完成。数据面清单 ADR + `storage/connect.py` central connector + SQLCipher 4.12 PoC（临时目录，不真迁移）；所有后端 SQLite 打开路径已收口，静态守卫禁止重新绕过。
