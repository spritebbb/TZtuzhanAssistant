...
> 截至本文：原功能路线图 **23/24 完成**，仅剩依赖公网资源的 D8。长期进化路线 M0–M5 的核心退出标准已达成，M6 首批低成本原型完成，M8 已完成未来信件、30/100/365 天关系快照、双视角叙事（A 经历页+B 全活动感想栏）；M3 的世界观/观察日志与网页/EPUB 属按真实使用再评估的扩展项。当前工作区验证基线为后端聚合 **66/66**、前端 Vitest **59/59**、人格 golden set **48 场景**、生产构建与浏览器关键路径 **6/6**。M1 临时对话、事实自然衰减与「长期保留」入口已完成；每日批处理 off-by-one 与主动统一仲裁器均已收口。
...
### M3.4 共同创作
- M3.4 首切片「轮流续写」已于 2026-09-06 完成（ZCode）：`backend/core/cowriting.py` + `/api/writings` + ActivityPanel 创作区；co_story 版本化 artifact，虚构隔离见 `cowriting_context` 的 `<fiction_story>` 声明
- 歌单/书单（共同清单）已于 2026-09-06 完成（ZCode）：colists + /api/lists + ActivityPanel 清单区；剩余类型：世界观/角色设定、观察日志（顺延切片）
- E03 关系导出/恢复已于 2026-09-06 落地（ZCode）：`/api/relationship/*` + kv 登记表（新 kv 键必须登记）+ MemoryPanel「带走 / 恢复」；恢复只允许空命名空间，主键重映射 + 引用重建
- M3.3 共同目标已于 2026-09-06 完成：动机/下一步/真实进展、陪做/单次提醒、过程回顾与 Markdown 导出
- M3 于 2026-09-06 完成退出核对（Codex / GPT-5）：共读补齐暂停/取消/导出和内容相关提问，专注补齐导出；所有活动的持久状态、语境门控和用户确认发送契约均有回归覆盖
...
- 当前聚合基线为 **66/66**；`test_edge_regressions.py` 另有快速边界回归
...
1. **先跑全量测试确认基线**：`.venv\\Scripts\\python.exe -m pytest tests\\test_suite_runner.py -q` → 当前基线应 **66/66 全绿**；前端执行 `npm test` 与 `npm run test:e2e`（当前分别 59/59、7/7）。
2. 读 `docs/EVOLUTION-ROADMAP.md`（逐项验收标准）+ `persona-菟菚.md`（人格约束，改动别破坏人设）
3. **下一推荐切片为 M8「阶段封存与告别」**：双视角 A/B 均已落地（dual_perspectives schema v12 + viewpoints 泛化到全部活动类型，`POST /api/activities/{id}/viewpoint-draft` 生成菟菚感想草稿）；封存=选择性导出（E03）超集，恢复路径已验证。剩余：梦境/平行可能性、重逢、不同版本的我们。M3 世界观/观察日志、网页/EPUB 按真实使用再评估；桌面宠物与 STT 仍需用户拍板。
