...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线已启动 Sprint 0：后端聚合基线 **45/45 全绿**、前端 Vitest **19/19**、浏览器关键路径 **5/5**，数据库升级前快照与恢复冒烟已落地；M0 只剩既有部署产物工作树收尾。
...
3. **完成 M0 再进 M1**：明确既有部署产物删除的处置归属；之后开始记忆溯源最小闭环。
