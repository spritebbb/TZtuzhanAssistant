...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线的 **M0 已完成**；**M1 记忆溯源最小闭环已完成（Sprint 1 4/4）**：facts 溯源、管理页“不主动提起”、冲突待确认与无残留清理均已落地。当前基线为后端聚合 **45/45 全绿**、前端 Vitest **22/22**、浏览器关键路径 **5/5**。
...
3. **进入 Sprint 2 关系事件内核**：先建立 `relationship_events` 与 4 类确定性事件，复用 M1 的来源、状态和级联清理原则；M1 的扩展隐私语义、导出恢复与自然遗忘留在后续增量完成。
