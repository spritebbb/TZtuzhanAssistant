...
> 截至本文：**路线图 21/23 完成**，仅剩 C5 养成仪表盘、D3/D8 探索项。基线测试 **41/41 全绿**。
...
**人格与玩法体系已基本完整**：记忆/心情/好感度/精力/约定/纪念日/日记/研究报告/知识库/解锁彩蛋/多模型路由/成本面板全链路打通。剩下的是**收尾与探索**（C5 仪表盘、D3 共同活动、D8 移动端）。
...
**陪伴机制**：日记+研究报告、纪念日预谋、约定提炼与跟进、**C4 好感度玩法闭环**（6 阈值解锁 + 3 彩蛋 + 「我们之间」收集 tab）
...
### C5 养成仪表盘（唯一"纯工程"主线项）
把好感度/精力/心情/约定/账本/收集串成总览页。
- **阻碍（roadmap 记录）**：`mood` 无历史表（users 表覆盖写），画曲线需新建 `mood_log`；前端无图表库需引依赖
- 数据来源：`affection_log`（delta/reason/ts 够画好感度曲线）、`usage_log`、`diary`、`unlocks` 收集、精力字段

### D3 共同活动（探索项，落地前细化）
陪伴终局不是聊天而是"一起做点什么"：共享 todo、一起读文聊感想等。工具链已够，缺玩法封装。
...
- **当前应 41/41 全绿**（比 D2 后的 40/40 多 1，因 C4 的 test_unlocks.py）
...
1. **先跑全量测试确认基线**：前缀命令 → 应 **41/41 全绿**
2. 读 `docs/EVOLUTION-ROADMAP.md`（逐项验收标准）+ `persona-菟菚.md`（人格约束，改动别破坏人设）
3. 若做 **C5**：按第五节阻碍 → 建 `mood_log` 历史表 → 引图表库 → 串好感度/精力/心情/账本/收集 → 新面板 → 测试 → 更新路线图 → commit
4. **D3/D8 两个探索项：不要直接开做**，需先与用户对齐方案（D8 还卡公网部署前置）。做之前先讨论。

---

_需要人工/用户决策的事项：C5 仪表盘的信息架构与图表库选型、D3 共同活动的玩法定位、D8 是否需要公网部署资源。_
