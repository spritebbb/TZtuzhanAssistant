...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线：**M0–M5 均已达成各自退出标准**（Sprint 1/2/3 + M4.1/M4.2 + M5.1，逐切片独立提交）；M4 遗留互动气质/幽默记忆/领域信任、M5 遗留低频生活事件/惊喜编排，均已按 Epic 立项待后续切片。当前基线为后端聚合 **49/49 全绿**、前端 Vitest **25/25**、浏览器关键路径 **5/5**。
...
3. **M0–M5 已全部达成退出标准**（2026-09-05）。下一大点是 M6「在场感与共同空间」：首批切片建议做"我们的角落"（把 artifacts/关系物件变成空间展示）+ 情绪声线（TTS 语速/音高随心情）；桌面宠物与语音输入需用户先拍板（见 `docs/TECH-PLAN.md` 第 10 节）。M4/M5 的遗留 Epic 见各自进度清单。
