...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线的 **M0 已完成**：后端聚合基线 **45/45 全绿**、前端 Vitest **19/19**、浏览器关键路径 **5/5**，数据库升级前快照与恢复冒烟已落地，旧发布产物已按现行策略退库。
...
3. **进入 M1 记忆溯源最小闭环**：facts 增加来源、置信度与验证时间，先做兼容迁移和后端契约，再接管理界面。
