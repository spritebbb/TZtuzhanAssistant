...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线：**M0–M5 均已达成各自退出标准，M6 首批低成本原型完成**（「我们的角落」artifact 空间 + 情绪声线 + 减动效；桌面宠物与 STT 待用户拍板），全部逐切片独立提交。当前基线为后端聚合 **50/50 全绿**、前端 Vitest **28/28**、浏览器关键路径 **5/5**。
...
3. **M6 首批已完成（2026-09-05）**：桌面宠物与 STT 需用户先拍板（`docs/TECH-PLAN.md` 第 10 节）；拍板前可继续的切片包括——M6 物件类型扩展（约定完成/纪念日 artifact）、共同审美、M4 遗留（互动气质/幽默记忆/领域信任）、M5 遗留（低频生活事件/惊喜编排）、M1 遗留（导出恢复/自然遗忘）。
