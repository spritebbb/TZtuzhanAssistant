...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线的 **M0 已完成**；**M1 记忆溯源最小闭环已完成（Sprint 1 4/4）**；**M2 关系事件内核首批 4 类事件已完成（Sprint 2）**；**M3 共读 2.0 已完成（Sprint 3）**；**M4 已开始：M4.1 关系季节完成**（真实信号确定性推导，见 `backend/core/seasons.py`）。当前基线为后端聚合 **47/47 全绿**、前端 Vitest **24/24**、浏览器关键路径 **5/5**。
...
3. **M4 进行中（M4.1 关系季节已完成）**：下一切片在 M4.2「双向了解」（用户可查看她的稳定偏好/雷区）与「冲突→修复多轮确定性回归」之间选择，或转 M5「自主生活」（`pending_thoughts` + Narrative Planner 也仍未做）；建议先读 `docs/TECH-PLAN.md` M4/M5 的退出标准再拍板。M1 的扩展隐私语义、导出恢复与自然遗忘同样留在后续增量。
