...
> 截至本文：原功能路线图 **23/24 完成**，仅剩 D8 移动端推送。长期进化技术路线的 **M0 已完成**；**M1 记忆溯源最小闭环已完成（Sprint 1 4/4）**；**M2 关系事件内核首批 4 类事件已完成（Sprint 2）**；**M3 共读 2.0 已完成（Sprint 3）**；**M4 退出标准已达成（M4.1 关系季节 + M4.2 双向了解/修复回归/偏好主权）**。当前基线为后端聚合 **48/48 全绿**、前端 Vitest **25/25**、浏览器关键路径 **5/5**。
