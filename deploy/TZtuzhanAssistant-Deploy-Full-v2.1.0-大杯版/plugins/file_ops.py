...
async def _read_file(path: str = "") -> str:
    ...
    try:
        ...
        try:
            ...
            return tool_failure(f"（{path} 不是文本文件，请改用 read_file_binary）")
...
async def _list_dir(path: str = "") -> str:
    """列出目录内容。"""
    p = _resolve_path(path) if path else _resolve_path(str(_WORK_DIR))
    if p is None:
        return tool_failure("（路径不允许）")
    try:
        if not p.exists():
            return tool_failure(f"（目录不存在: {path}）")
        if not p.is_dir():
            return tool_failure(f"（{path} 不是目录）")
        items = []
        ...
        return tool_failure("（无权限访问）")
    except Exception as e:
        return tool_failure(f"（列出目录失败：{e}）")
