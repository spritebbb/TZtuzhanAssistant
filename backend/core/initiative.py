...
def _proactive_done_today(user_id: str) -> bool:
    ...
    return kv_get(user_id, _proactive_key(user_id, today)) is not None


def _mark_proactive(user_id: str) -> None:
    today = datetime.date.today().isoformat()
    kv_set(user_id, _proactive_key(user_id, today), "1")
...
def _eligible_users() -> list[dict]:
    ...
    for r in rows:
        ...
        if last is not None and now - last < _IDLE_HOURS * 3600:
            continue
...
def _build_proactive_prompt(user_id: str) -> list[dict] | None:
...
async def _tick_once() -> int:
    ...
    now = time.time()
    if now - _last_global_run < _GLOBAL_COOLDOWN_SEC:
        return 0
...
# ---- 投递通道（可注入）----
_deliver_hook = None  # async (user_id, text) -> bool


def set_deliver_hook(hook) -> None:
    """注册主动消息投递回调。hook 签名：async (user_id, text) -> bool（是否投递成功）。"""
    global _deliver_hook
    _deliver_hook = hook


async def _deliver(user_id: str, text: str) -> bool:
    if _deliver_hook is None:
        return False
    try:
        return bool(await _deliver_hook(user_id, text))
    except Exception:
        return False
...
def dequeue_proactive(user_id: str) -> str | None:
    """取出并删除该用户的待投递消息；无则返回 None。"""
    text = kv_get(user_id, _PENDING_KEY)
    if text is None:
        return None
...
def _notify_subscribers(user_id: str, text: str) -> None:
...
async def sse_event_stream(user_id: str):
    ...
    try:
        ...
        pending = dequeue_proactive(user_id)
        if pending:
