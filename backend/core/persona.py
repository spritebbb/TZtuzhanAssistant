    ...
    *,
    ...
    if behavior_text is not None:
        ...
        try:
            ...
            cal = effective_modulation(
                user_id, datetime.now().date(),
                energy=_state.energy, tension=_state.tension,
            )
            _behavior = _frame(
                _state, season_line=_season(user_id, _state)["line"],
                calendar_line=compose_line(cal),
            )
            behavior_line = (
    ...
    if user_id:
        try:
            ...
            try:
                ...
                _emotions = active_emotions_map(user_id)
            except Exception:
                _emotions = {}
    ...
    # M4 观点连续性：允许友好分歧，但不瞬间迎合推翻，也不把偏好当对方事实
    notes.append(
        "你有自己稳定的偏好和观点：对方不同意时可以友好分歧、说出你的理由，"
        "接受有依据的新看法时说明是什么让你改变；"
        "不要为了迎合对方而瞬间推翻你一直持有的看法，更不要把你的偏好说成对方的客观事实。"
    )
    ...
    # 「晒太阳/阳光」日常描述约束已写入人格卡「说话风格」第 5 条（单点维护），
    # 不再由代码重复注入，避免卡与代码两处措辞漂移。
