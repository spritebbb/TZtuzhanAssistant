...
- 🎓 **内置能力演示（新手教程）**：说「新手教程」或点「更多 → 能力演示」，她按能力域分组**真实执行** 18 步能力展示（记忆/联网/多模态/动手/Agent 主秀/长期陪伴），每步停下等你决定是否继续，也可以点名跳步。
...
- **聊天里说一句**：「新手教程」「演示一下」「带我看看你会什么」——她会按顺序**真实执行**演示，每步做完停下等你决定是否继续；也可以点名跳步（「直接演示生图」）；
- **点界面入口**：顶部「更多 → 能力演示」，按能力域分组，每步都有一句可直接发送的原话；界面操作类步骤会标出要点哪个按钮。

演示共 **18 步，按能力域分组**（从日常到硬核）：

| 分组 | 展示 |
|---|---|
| 记忆与了解 | 长期记忆：说记住就真的记得住 |
| 联网与信息 | 联网查实时信息：拿的是今天的数据 |
| 多模态 | 生图、识图（发图给她看）、语音朗读 |
| 动手与执行 | 读写本机文件、跑代码算结果、汇率换算、任务面板 |
| **Agent 主秀** | **并行子代理**、**浏览器自动化（MCP）**、查实时文档（MCP）、监控页面、定时任务、**多步任务 + 产物落盘** |
| 长期陪伴 | 情绪与关系状态、可解释性（她为什么这么说）、临时对话（不留痕） |

Agent 主秀的完整原话与验收点见 [docs/AGENT-TOUR.md](docs/AGENT-TOUR.md)。
