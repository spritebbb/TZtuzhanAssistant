...
def _run_import(data_root: Path, code: str = "from backend.core.userdb import db") -> None:
    env = os.environ.copy()
    env.update({
...
def test_v40_upgrade_and_repeat_idempotency() -> tuple[Path, Path]:
    ...
    backups = list((data_root / "backups").glob("schema-bot-v40-to-v41-*/bot.db"))
    ...
    _run_import(data_root)
    after_backups = list((data_root / "backups").glob("schema-bot-v40-to-v41-*/bot.db"))
    conn = sqlite3.connect(db_path)
