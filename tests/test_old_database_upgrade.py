...
TARGET_VERSION = 43  # v43: situation files（D9 局势档案）
...
def test_v40_upgrade_and_repeat_idempotency() -> tuple[Path, Path]:
    ...
    backups = list((data_root / "backups").glob("schema-bot-v40-to-v43-*/bot.db"))
    ...
    _run_import(data_root)
    after_backups = list((data_root / "backups").glob("schema-bot-v40-to-v43-*/bot.db"))
    conn = sqlite3.connect(db_path)
